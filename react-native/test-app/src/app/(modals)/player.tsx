...
export default function PlayerModal() {
  ...
  const [trackWidth, setTrackWidth] = React.useState(0); // px
  const [dragging, setDragging] = React.useState(false);
  const [dragPct, setDragPct] = React.useState(0); // 0..100

  const clamp = (v: number, min = 0, max = 100) => Math.max(min, Math.min(max, v));

  const displayedPct = dragging ? dragPct : getProgress();
  const displayedSeconds = React.useMemo(() => {
    if (!duration) return 0;
    return (displayedPct / 100) * duration;
  }, [displayedPct, duration]);
  ...
  const panResponder = React.useMemo(
    () =>
      PanResponder.create({
        ...
        onPanResponderGrant: (evt) => {
          ...
          const x = (evt.nativeEvent as any).locationX || 0;
          const pct = clamp((x / trackWidth) * 100);
          setDragging(true);
          setDragPct(pct);
        },
        ...
        onPanResponderRelease: async () => {
          ...
          const targetSeconds = (dragPct / 100) * duration;
    ...
    [trackWidth, duration, dragPct, seekTo]
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {!!error && (
        <ErrorBanner
          message={error}
          onRetry={() => currentTrack && play(currentTrack)}
          onClose={clearError}
        />
      )}
        ...
        <View style={styles.content}>
          ...
          >
            ...
            >
              <View style={styles.progressBackground}>
                {/* Fill */}
                <View style={[styles.progressFill, { width: `${displayedPct}%` }]} />
                {/* Thumb */}
                <View
                  style={[
                    styles.progressThumb,
                    { left: `${displayedPct}%`, backgroundColor: "white" },
                  ]}
                />
              </View>
            </View>
            ...
            {/* Time Display */}
            <View style={styles.timeContainer}>
              <Text style={styles.timeText}>{formatTime(displayedSeconds)}</Text>
              <Text style={styles.timeText}>{formatTime(duration)}</Text>
            </View>

            {/* Progress Percentage */}
            <Text style={styles.progressPercentage}>{Math.round(displayedPct)}% Complete</Text>
          </GlassCard>
...
const styles = StyleSheet.create({
  ...
  progressTrack: {
    marginBottom: THEME.spacing.md,
    paddingVertical: 8, // dokunma alanını büyüt
  },
  progressBackground: {
    height: 8,
    backgroundColor: "rgba(255, 255, 255, 0.1)",
